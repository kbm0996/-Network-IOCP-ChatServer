...
unsigned int CChatServer::UpdateThread(LPVOID pCNetServer)
{
	return ((CChatServer*)pCNetServer)->UpdateThread_Process();
}
...
void CChatServer::Proc_Heartbeat()
{
	for (auto iter = _PlayerMap.begin(); iter != _PlayerMap.end(); ++iter)
	{
		...
		if (GetTickCount64() - pPlayer->LastRecvTick > en_HEART_BEAT_INTERVAL)
