...
bool mylib::CLanClient::Start(WCHAR * wszConnectIP, int iPort, int iWorkerThreadCnt, bool bNagle)
{
	...
	{
		LOG(L"SYSTEM", LOG_ERROR, L"connect() ErrorCode:%d", WSAGetLastError());
	...
	if (_hIOCP == NULL)
	{
		LOG(L"SYSTEM", LOG_ERROR, L"CreateIoCompletionPort() ErrorCode:%d", WSAGetLastError());
		closesocket(_pSession->Socket);
		_pSession->Socket = INVALID_SOCKET;
		WSACleanup();
		return false;
	...
	if (CreateIoCompletionPort(reinterpret_cast<HANDLE>(_pSession->Socket), _hIOCP, reinterpret_cast<ULONG_PTR>(_pSession), 0) == NULL)
	{
...
bool mylib::CLanClient::SendPacket(CNPacket * pPacket)
{
	...
	pPacket->SetHeader_Custom(reinterpret_cast<char*>(&wHeader), sizeof(wHeader));
...
mylib::CLanClient::stSESSION * mylib::CLanClient::ReleaseSessionLock()
{
...
bool mylib::CLanClient::ReleaseSession()
{
	...
	if (!InterlockedCompareExchange128((LONG64*)_pSession->stIO, TRUE, 0, (LONG64*)&stComparentIO))
		return false;
	...
	DisconnectSocket(_pSession->Socket);
...
int mylib::CLanClient::DisconnectSocket(SOCKET socket)
{
	return shutdown(socket, SD_BOTH);
}

bool mylib::CLanClient::DisconnectSession()
{
	stSESSION *pSession = ReleaseSessionLock();
	if (pSession == nullptr)
		return false;
...
void mylib::CLanClient::RecvComplete(DWORD dwTransferred)
{
	...
	while (1)
	{
		...
		if (wHeader > CNPacket::en_BUFFER_DEFAULT_SIZE)
		{
		...
		_pSession->RecvQ.Dequeue(pPacket->GetPayloadPtr(), wHeader);
...
void mylib::CLanClient::SendComplete(DWORD dwTransferred)
{
	...
	if (_pSession->bSendDisconnect == TRUE && _pSession->SendQ.GetUseSize() == 0)
	{
		DisconnectSocket(_pSession->Socket);
		return;
	}
...
mylib::CLanClient::stSESSION::stSESSION()
{
	...
	stIO = reinterpret_cast<stIOREF*>(_aligned_malloc(sizeof(stIOREF), 16));
