...
int mylib::CLanServer::DisconnectSocket(SOCKET socket)
{
...
bool mylib::CLanServer::RecvPost(stSESSION * pSession)
{
	...
	if (WSARecv(pSession->Socket, wsabuf, iBufCnt, &dwTransferred, &dwFlag, &pSession->RecvOverlapped, NULL) == SOCKET_ERROR) // 성공 : 0, 실패 : SOCKET_ERROR
	{
		...
		if (err != WSA_IO_PENDING)
		{
			...
			if (err != WSAENOTSOCK && err != WSAECONNABORTED && err != WSAECONNRESET && err != WSAESHUTDOWN)
...
void mylib::CLanServer::RecvComplete(stSESSION * pSession, DWORD dwTransferred)
{
	...
	while (1)
	{
		...
		{
			LOG(L"SYSTEM", LOG_WARNG, L"PacketBufferSize < PayloadSize ", pSession->iSessionID);
			DisconnectSocket(pSession->Socket);
			return;
		}
		...
		pSession->RecvQ.Dequeue(pPacket->GetPayloadPtr(), wHeader);
